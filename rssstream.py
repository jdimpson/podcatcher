...
class RssStreamParser(object):
	def __init__(self,url, text=None, resp=None, type=None, verb=False, ua=None, timeout=None, raize=False):
		self.bail = False
		self.raize = raize
		self.verb = verb
		self.url = url
		...
		if text is not None:
			...
			self.resp = requests.get(self.url,stream=True,headers=hd,timeout=timeout)
			self.text = ''
			self.check_resp()
